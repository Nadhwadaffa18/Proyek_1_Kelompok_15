<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center gap-4 w-full">
            ...
            <a href="{{ route('admin.classes.index') }}" class="flex-shrink-0 inline-flex items-center text-sm font-semibold text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 transition-colors">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white dark:bg-slate-900 rounded-[18px] border border-slate-200/80 dark:border-slate-800/80 shadow-soft overflow-hidden">
                    <div class="px-6 py-4 border-b border-slate-200/80 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50">
                        <h3 class="font-bold text-slate-800 dark:text-white">Tambah Jadwal</h3>
                        <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Atur hari dan jam kelas</p>
                    </div>
                    <div class="p-6">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-[18px] border border-slate-200/80 dark:border-slate-800/80 shadow-soft overflow-hidden">
                    <div class="px-6 py-5 border-b border-slate-200/80 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50">
                        <h3 class="font-bold text-slate-900 dark:text-white">
                            Jadwal Aktif
                            <span class="ml-2 px-2.5 py-0.5 rounded-full text-[10px] font-bold font-mono bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/10">
                    ...
                    <div class="divide-y divide-slate-100 dark:divide-slate-800/60">
                        @forelse ($class->schedules as $schedule)
                            @php
                                $dayColors = [
                                    'Senin'  => 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/10',
                                    'Selasa' => 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/10',
                                    'Rabu'   => 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/10',
                                    'Kamis'  => 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/10',
                                    'Jumat'  => 'bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/10',
                                    'Sabtu'  => 'bg-teal-500/10 text-teal-600 dark:text-teal-400 border-teal-500/10',
                                    'Minggu' => 'bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/10',
                                ];
                                $color = $dayColors[$schedule->day] ?? 'bg-slate-500/10 text-slate-600 dark:text-slate-400 border-slate-500/10';
                            @endphp
                            <div class="flex items-center justify-between p-5 hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p class="font-bold text-slate-900 dark:text-white text-sm font-mono">
                                            {{ substr($schedule->start_time, 0, 5) }} – {{ substr($schedule->end_time, 0, 5) }}
                                        </p>
                                        <p class="text-[10px] font-semibold mt-1">
                                            @if ($schedule->meeting_link)
                                                <span class="text-emerald-600 dark:text-emerald-400">✅ Link GMeet sudah diset guru</span>
                                            @else
                                                <span class="text-slate-400 dark:text-slate-500 italic font-normal">Belum ada link dari guru</span>
                                            @endif
                                        </p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-2.5 shrink-0">
                                    <a href="{{ route('admin.schedules.edit', [$class->id, $schedule->id]) }}"
                                       class="px-3 py-1.5 text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-500/10 border border-indigo-500/10 rounded-lg hover:bg-indigo-600 hover:text-white transition-all">
                                        Edit
                                    </a>
                                    <form action="{{ route('admin.schedules.destroy', [$class->id, $schedule->id]) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="px-3 py-1.5 text-xs font-bold text-rose-600 dark:text-rose-400 bg-rose-500/10 border border-rose-500/10 rounded-lg hover:bg-rose-600 hover:text-white transition-all">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
                        ...
                        @empty
                            <div class="p-16 text-center">
                                ...
                                <p class="text-sm font-bold text-slate-700 dark:text-slate-300">Belum ada jadwal</p>
                                <p class="text-xs text-slate-400 dark:text-slate-500 mt-1.5">Tambahkan jadwal menggunakan form di sebelah kiri.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
