                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 text-sm text-slate-600 dark:text-slate-300">
                            @forelse ($classes as $class)
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors duration-150">
                                    <td class="py-4 font-semibold text-slate-900 dark:text-white">{{ $class->name }}</td>
                                    <td class="py-4 font-medium">
                                        @if($class->teacher)
                                            <span class="text-slate-700 dark:text-slate-200">{{ $class->teacher->name }}</span>
                                        @else
                                            <span class="text-slate-400 dark:text-slate-500 italic">Belum Ditentukan</span>
                                        @endif
                                    </td>
                                    <td class="py-4 font-mono text-xs text-slate-400 dark:text-slate-500">{{ $class->created_at->format('d M Y') }}</td>
                                    <td class="py-4 text-right space-x-3.5">
                                        <a href="{{ route('admin.classes.students', $class->id) }}" class="inline-flex items-center text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:underline">
                                            Siswa
                                        </a>
                                        <a href="{{ route('admin.schedules.index', $class->id) }}" class="inline-flex items-center text-xs font-bold text-amber-600 dark:text-amber-400 hover:underline">
                                            Jadwal
                                        </a>
                                        <a href="{{ route('admin.classes.edit', $class->id) }}" class="inline-flex items-center text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:underline">
                                            Edit
                                        </a>
                                        <form action="{{ route('admin.classes.destroy', $class->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus kelas ini?')" class="inline-flex items-center text-xs font-bold text-rose-600 dark:text-rose-400 hover:underline">
                                                Hapus
                                            </button>
                                        </form>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-8 text-center text-slate-400 dark:text-slate-500">Belum ada kelas yang dibuat.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
