<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        @endif
        @if (session('error'))
            <div class="bg-rose-500/10 border border-rose-500/20 text-rose-600 p-4 rounded-xl shadow-sm backdrop-blur-md flex items-center gap-3">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="relative max-w-sm">
        <input
            ...
            class="w-full rounded-xl border border-slate-200 pl-10 pr-4 py-2.5 focus:border-primary focus:ring-primary"
        >

        <i class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"></i>
    </div>
</form>
        <div class="bg-white rounded-[18px] border border-slate-200 shadow-soft overflow-hidden">
            <div class="p-6">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-200 text-sm text-slate-600">
                                ...
                                <tr class="hover:bg-slate-100 transition-colors duration-150">
                                    ...
                                    <td class="py-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold font-mono uppercase tracking-wider
                                            @if($user->role === 'admin') bg-indigo-500/10 text-indigo-600  border border-indigo-500/10
                                            @elseif($user->role === 'guru') bg-emerald-500/10 text-emerald-600  border border-emerald-500/10
                                            @else bg-amber-500/10 text-amber-600  border border-amber-500/10
                                            @endif">
                                            {{ $user->role }}
                                        </span>
                                    </td>
                                    <td class="py-4 text-right space-x-3 whitespace-nowrap">
                                        ...
                                        @if ($user->id !== auth()->id())
                                            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus pengguna ini?')" class="inline-flex items-center text-sm font-semibold text-rose-600 hover:text-rose-800">
                                                    Hapus
                                                </button>
                                            </form>
                                        @endif
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-8 text-center text-slate-400 ">Belum ada pengguna.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
