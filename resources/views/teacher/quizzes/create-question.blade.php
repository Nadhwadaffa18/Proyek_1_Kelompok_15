<x-app-layout>
    ...
    <div class="max-w-xl mx-auto">
        <div class="bg-white rounded-[18px] border border-slate-200/80 shadow-soft overflow-hidden">
            <div class="p-8">
                <h3 class="font-bold text-xs text-slate-400 mb-6 uppercase tracking-wider font-mono">Kuis: {{ $quiz->title }}</h3>
                ...
                <form action="{{ route('teacher.quizzes.questions.store', $quiz->id) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="question" name="question" rows="4" class="block mt-1.5 w-full bg-white  text-slate-900 border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-[12px] shadow-sm py-2 px-3.5 transition-all text-sm placeholder-slate-400 outline-none" required autofocus placeholder="Tuliskan pertanyaan di sini...">{{ old('question') }}</textarea>
                        <x-input-error :messages="$errors->get('question')" class="mt-2" />
                    </div>

                    <!-- Pilihan Jawaban -->
                    <div class="space-y-4 pt-4 border-t border-slate-100">
                        <h4 class="font-bold text-xs text-slate-400 uppercase tracking-wider font-mono">Pilihan Jawaban (Multiple Choice)</h4>
                    ...
                    <div class="pt-4 border-t border-slate-100">
                        ...
                        <select id="answer" name="answer" class="block mt-1.5 w-full bg-white text-slate-900 border border-slate-200  focus:border-indigo-500  focus:ring-1 focus:ring-indigo-500 rounded-[12px] shadow-sm py-2.5 px-3.5 transition-all text-sm" required>
                    ...
                    <div class="flex items-center justify-end space-x-4 pt-4 border-t border-slate-100">
                        <a href="{{ route('teacher.quizzes.show', $quiz->id) }}" class="text-sm font-semibold text-slate-500 hover:text-slate-800 transition-colors">
                            Batal
                        </a>
                        <x-primary-button>
                            Simpan Soal
