<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            ...
            <a href="{{ route('teacher.classes.show', $class->id) }}" class="inline-flex items-center text-sm font-semibold text-slate-500 hover:text-slate-800 transition-colors">
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        {{-- Info box --}}
        <div class="bg-blue-500/5 border border-blue-500/10 rounded-xl px-5 py-4 text-sm text-blue-700 flex items-start gap-3">
            <svg class="w-5 h-5 shrink-0 text-blue-500 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
                <strong class="font-bold">Cara pakai:</strong> Sebelum kelas dimulai, buat link Google Meet baru lalu tempel di kolom jadwal yang sesuai. Siswa akan langsung melihat tombol <span class="font-semibold text-slate-800">Masuk Kelas</span> di halaman kelas mereka.
            </div>
        </div>
            ...
            <div class="bg-white rounded-[18px] border border-slate-200/80 shadow-soft overflow-hidden">
                ...
                <div class="p-6">
                    <form action="{{ route('teacher.schedules.updateLink', [$class->id, $schedule->id]) }}" method="POST" class="space-y-3">
                        @csrf
                        @method('PATCH')
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wide font-mono mb-1">
                        ...
                        <div class="flex gap-3">
                            <input type="url" name="meeting_link"
                                ...
                                class="flex-grow bg-white text-slate-900 border border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-[12px] shadow-sm py-2 px-3.5 transition-all text-sm outline-none" required>
        ...
        @empty
            <div class="bg-white rounded-[18px] border border-slate-200/80 p-16 text-center shadow-soft">
                ...
                <p class="font-bold text-slate-700">Belum ada jadwal</p>
                <p class="text-xs text-slate-400 mt-1.5">Admin belum mengatur jadwal untuk kelas ini.</p>
            </div>
        @endforelse
    </div>
</x-app-layout>
